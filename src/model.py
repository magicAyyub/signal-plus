...
class DETR(nn.Module):
    ...
    def load_pretrained_from_hf(self, repo_id: str, filename: str = "4426_model.pt", device=None):
        """
        Load pretrained weights from Hugging Face Hub.
        
        Args:
            repo_id: Hugging Face repo ID (e.g., 'username/signdetr-pretrained')
            filename: Name of the checkpoint file in the repo
            device: Device to load the model to
        
        Example:
            model.load_pretrained_from_hf('yourusername/signdetr-pretrained', '4426_model.pt')
        """
        try:
            ...
            # Download the file from HF Hub (will cache it locally)
            ...
            # Load the checkpoint
            self.load_state_dict(torch.load(checkpoint_path, map_location=device, weights_only=False))
            self.model_handler.log_model_loading(f"{repo_id}/{filename}", success=True)
            
        except ImportError:
            self.logger.error("huggingface_hub not installed. Run: pip install huggingface_hub")
            self.model_handler.log_model_loading(f"{repo_id}/{filename}", success=False)
        except Exception as e:
            self.logger.error(f"Failed to load checkpoint from HF: {str(e)}")
            self.model_handler.log_model_loading(f"{repo_id}/{filename}", success=False)
